...
SOURCES = {
    "client": "Project supplied",
    "dune": "Dune API",
    "graphql": "Subghraph index",
    "near": "Near blockchain",
    "regen": "Regen blockchain",
}
...
class ProjectMetricMeta(models.Model):
    ...
    category = models.ForeignKey(
        Category, on_delete=models.SET_NULL, null=True, blank=True, related_name="meta"
    )
    source = models.CharField(
        max_length=255, default=SOURCES["client"], choices=SOURCES
    )
...
class ProjectMetric(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        try:
            prev = ProjectMetric.objects.values_list("value", flat=True).get(
                pk=self.pk
            )  # pull the existing value straight from the DB
            # wrap in a transaction so that both the save and the delta‐log happen atomically
            with transaction.atomic():
                ProjectMetricDelta.objects.create(
                    meta=self.meta, value=self.value - prev
                )
                return super().save(
                    update_fields=["value", "timestamp"], *args, **kwargs
                )
...
class ProjectMetricDelta(models.Model):
    ...
    meta = models.ForeignKey(
        ProjectMetricMeta, on_delete=models.CASCADE, related_name="deltas"
    )
    value = models.FloatField()
    timestamp = models.DateTimeField(default=timezone.now)
...
class AggregateMetric(models.Model):
    ...
    project_metrics = models.ManyToManyField(
        ProjectMetric, related_name="aggregate_metrics"
    )

    # Optional category filter
    categories = models.ManyToManyField(
        Category, blank=True, related_name="aggregate_metrics"
    )
    ...
    aggregation_method = models.CharField(
        max_length=10, choices=AGGREGATION_CHOICES, default="SUM"
    )
    ...
    def get_contributing_projects(self):
        ...
        project_ids = self.project_metrics.values_list(
